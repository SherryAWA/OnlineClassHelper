...
class GetCatalog(baseGetJson):
    ...
    def onlyTest(self,chapter):
        cnt=0
        task_cnt=0
        if(chapter['task_id']!=0): return True
        for section in chapter['children']:
            children=section.get('children',[])
            if len(children)==0:
                if section['status'] != "0" and section['finished'] == 0:
                    cnt+=1
                    if section['task_id']!=0:
                        task_cnt+=1
            else:
                for child in children:
                    if child['status'] != "0" and child['finished'] == 0:
                        cnt+=1
                        if child['task_id']!=0:
                            task_cnt+=1
        return cnt==task_cnt
    def get_available_catalog_dict(self):
        ...
        for chapter in res['data']:
            # 该章可用且尚未完成且不仅含测验
             if chapter['status'] != "0" and chapter['finished'] == 0 and self.onlyTest(chapter)!=True:
            ...
            for chapter in self.catalog[course]:
                for section in self.catalog[course][chapter]:
                    ...
                    for video in res['data']:
                        a=video.get('video_url',0)
                        if a==0: 
                            continue
                            # 视频未完成
                        if video['finished'] == 0:
                            self.catalog[course][chapter][section] = []
                            self.catalog[course][chapter][section].append({'id': video['id'], 'title':video['title'], 'video_url':video['video_url'],
...
if __name__ == '__main__':
    ...
    cookie = 'Hm_lvt_d1a5821d95582e27154fc4a1624da9e3=1745934568,1745936742,1746088623,1747714663; HMACCOUNT=B1CC654DB75C7919; uooc_auth=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJyZW1lbWJlciI6dHJ1ZSwibG9naW5faG9zdCI6InVvb2MubmV0LmNuIiwic3ViMSI6MCwic3ViIjoiMTAxODk4OSIsImV4cCI6MTc2MzI2NjczMn0.hiiqeCQFTm2TeVQfUxZusvXy72KiUKPmK1HxXP4m4ww; account=13421821537; Hm_lpvt_d1a5821d95582e27154fc4a1624da9e3=1747714744; formpath=/home; formhash=/center/course/learn; tfstk=gynKem15PCA3G1nLIv8GrpCpwvJi2FDeBXkfq7b7AfIS1fo3PYvUFVhSsWGuxvj865Gr-zVkLzGtNf73EWvE6Lhmw84o2YS8FYGyL7veSvkU4uODiuKmLv8eYZypsu9SCu2-vlr7lvkU4O79IhtsLTFaqOZ7VbZsC828VJaQVdQ_E5sQFMZ71dFa1Ms5AzM15-2VO8G7VdH__8Z7dbaBBAGrLhFyG7I-uIbydfd0MgsSXJUQSvN1VRoTpyFjdaj5VOyLJSMQ6BtjbVzsUznPUgUodqlzhX1Bw7otCDgjGCb3Oqe-n4hXEwygyDrTNDtVH5iKecUES6_QHkFLlDadnKkUPVnTxDOPuADTOrEiSFR3rkhKuSzB7Cl-BWcSvP1ps70mIDatGCjtaz3SxliB1nIrpjcxOLf0M8bBBdQPzywauRc7CWjuNzyTidzOzazp_RFDBdQPzywaBSvaWa7zJC5..'
    # 定义好的网页接口
    url = {
